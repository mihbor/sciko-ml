...
class DecisionTreeRegressor {
    ...
    fun fit(X: MultiArray<Double, D2>, y: MultiArray<Double, D1>) {
        root = buildTree(X, y)
    }

    fun fit(X: DataFrame, y: DataFrame.Column<Double>) {
    ...
    fun predict(X: MultiArray<Double, D2>): MultiArray<Double, D1> {
        return mk.ndarray(X.toList().map { predict(it) })
    }
    ...
    private fun predict(x: List<Double>): Double {
        var node = root
